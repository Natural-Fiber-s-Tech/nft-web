...
// Old legacy components (FeatureCard, SpecRow, CapabilityItem, MediaCard) removed due to Flat Schema Migration
...
const PublicMediaCarousel = ({ product, getText }) => {
  ...
  const items = useMemo(() => {
    const res = [];
    if (product.video)
      res.push({ type: "video", url: product.video });
    if (product.photos) res.push({ type: "image", url: product.photos });
    return res.length ? res : [{ type: "image", url: product.photos }];
  }, [product]);
        ...
        <div className="flex justify-center space-x-2 mt-0">
          {items.map((_, i) => (
            <button
              key={i}
              onClick={() => setIdx(i)}
              className={`w-3 h-3 rounded-full ${i === idx ? "bg-red-600" : "bg-gray-300 hover:bg-gray-400"
                }`}
            />
          ))}
        </div>
...
export default function ProductDetailTemplate({
  ...
  // No more nested mappings in flat schema.
  ...
  return (
    <div className="min-h-[60vh]">
        ...
        <div className="span-12">
          <div className="mb-4">
            ...
            {editable ? (
              <div className="relative">
                <input
                  className={`text-lg text-red-600 font-medium mb-3 bg-transparent border rounded px-2 py-1 w-full ${invalid?.tagline ? "border-red-500 ring-1 ring-red-300" : ""
                    }`}
      ...
      <div className="grid-ctx mb-6 mt-8">
        <div className="span-12 lg:span-8 lg:col-start-3">
          <div className="h-96">
